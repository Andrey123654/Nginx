...
function render(report) {
  ...
  document.querySelector('#publications-list').innerHTML = report.publications.map(item => {
    ...
    const edgeRules = (item.registry_matches || []).length ? `<details class="setting-guide edge-rules"><summary>Сопоставленные правила Edge / DNAT (${esc(item.registry_matches.length)})</summary>${item.registry_matches.map(match => `<div><b>${esc((match.rule_ids || [match.rule_id]).join(', '))}<br>${esc(match.external_endpoint)} → ${esc(match.internal_endpoint)}/${esc(match.protocol)}</b><span>${(match.visibility || []).map(zone => visibilityName(zone)).join(', ')}. Applied on: ${esc((match.scope_names || []).join(', ') || 'не указано')}. Сопоставление: ${esc(match.confidence)}.</span></div>`).join('')}</details>` : '';
    ...
    return `<article class="publication-card">
      ...
      <div class="publication-meta"><div><b>Listen</b><span>${esc(item.listen.join(', '))}</span></div><div><b>Потенциальная зона</b><span>${declared}</span></div><div><b>Фактически по Edge / датчикам</b><span>${actual}</span></div><div><b>Адреса / upstream</b><span>${addresses}<br>${esc(item.upstreams.join(', ') || 'upstream не найден')}</span></div></div>${edgeRules}${settingHelp}
  ...
  registrySummary.innerHTML = [
    bundle ? `<b>Многофайловая выгрузка:</b> секций ${esc(bundle.sections)}, активных HTTP ${esc(bundle.http_sections)}, stream ${esc(bundle.stream_sections)}, отключённых/резервных пропущено ${esc(bundle.inactive_sections_ignored || 0)}.` : '',
    registry ? `<b>Выгрузка Edge / DNAT:</b> строк ${esc(registry.source_rows)}, уникальных трансляций ${esc(registry.total)}, объединено повторов ${esc(registry.collapsed_rules)}, сопоставлено ${esc(registry.matched)}, неоднозначно ${esc(registry.ambiguous)}, не относится к найденным Nginx listener ${esc(registry.unmatched)}. <span>Зоны: наружу ${esc(registry.zone_counts?.external || 0)}, внутрь ${esc(registry.zone_counts?.internal || 0)}, обе ${esc(registry.zone_counts?.both || 0)}, требуют уточнения ${esc(registry.zone_counts?.unknown || 0)}. ${esc(registry.matching_note)}</span>` : '',
