...
Версия 1.6.0 добавляет загрузку реестра внешних DNAT-публикаций `.xlsx` и раздельный анализ HTTP/stream в многофайловых выгрузках. После обновления обязательно повторно запустите установщик или пересоберите Docker-образ: зависимости `openpyxl` и `defusedxml` устанавливаются из обновлённого `requirements.txt`.

Версия 1.7.0 поддерживает актуальную выгрузку Edge с колонками `Applied on`, `Original IP/Port`, `Translated IP/Port`, объединяет повторяющиеся трансляции и раздельно определяет внешнюю и внутреннюю зоны. Старый XLSX-формат продолжает работать.
